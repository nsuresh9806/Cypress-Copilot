...
async function openAIcallStep(userApiKey, inputText, modelName) {
    try {
        const openai = new openai_1.default({ apiKey: userApiKey });
        const getStepCodeAPIcall = await openai.chat.completions.create({
            model: modelName,
            messages: [
                { role: "system", content: config_json_1.default.system_prompt_code },
                { role: "user", content: "for {feature} =" + config_json_1.default.few_shot_assistant_feature_code },
                { role: "assistant", content: "the example step definition javascript file is {step} = " + config_json_1.default.few_shot_assistant_step_code },
                {
...
async function openAIcallPom(userApiKey, inputText, modelName) {
    const openai = new openai_1.default({ apiKey: userApiKey });
    const getPomCodeAPIcall = await openai.chat.completions.create({
        model: modelName,
        messages: [
            { role: "system", content: config_json_1.default.system_prompt_code },
            { role: "user", content: "for the {step} file =" + config_json_1.default.few_shot_assistant_step_code },
            { role: "assistant", content: "the page object class implementation is {page} = " + config_json_1.default.few_shot_assistant_page_code },
            {
...
function isValidBDDFormat(inputText) {
...
async function activate(context) {
    ...
    const processtest = vscode.commands.registerCommand('ccaia.processTest', () => {
        ...
        let currentPanel = vscode.window.createWebviewPanel('processTests', 'process Tests', vscode.ViewColumn.One, { enableScripts: true, });
